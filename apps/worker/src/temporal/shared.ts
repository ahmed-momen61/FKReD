...
export interface TargetLedger {
  ...
  leakedCredentials: Array<{ username: string; hashOrPassword: string; source: string }>;
  activePivotNodes: Array<{ ip: string; port: number; protocol: string }>;
...
export interface PipelineInput {
  ...
  // Red Team Extensions
  targetIp?: string;
  scanDepth?: 'shallow' | 'deep' | 'aggressive';
...
export interface PipelineState {
  ...
  errorCode?: ErrorCode;
  startTime: number;
  agentMetrics: Record<string, AgentMetrics & { ledgerUpdates?: Partial<TargetLedger> }>;
  summary: PipelineSummary | null;
  
  // Red Team Core State
  ledger: TargetLedger;
  currentDepth: 'shallow' | 'deep' | 'aggressive';
}
...
export interface VulnExploitPipelineResult {
  ...
  error: string | null;
  ledgerUpdates?: Partial<TargetLedger>;
}
