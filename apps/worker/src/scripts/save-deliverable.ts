...
function parseArgs(argv: string[]): ParsedArgs {
  ...
  for (let i = 2; i < argv.length; i++) {
    const arg = argv[i];
    const next = argv[i + 1];
    if (arg === '--type' && next) { args.type = next; i++; } 
    else if (arg === '--content' && next) { args.content = next; i++; } 
    else if (arg === '--file-path' && next) { args.filePath = next; i++; }
  }
  return args;
}
...
function main(): void {
  ...
  if (args.content) {
    ...
    if (!resolved.startsWith(`${cwd}/`) && resolved !== cwd) {
      console.log(JSON.stringify({ status: 'error', message: `Path traversal detected: ${args.filePath}`, retryable: false }));
      process.exit(1);
    }
    try {
    ...
    console.log(JSON.stringify({ status: 'error', message: 'Either --content or --file-path is required', retryable: false }));
    process.exit(1);
  }
